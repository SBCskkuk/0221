...
public static class TrafficHQEditorGizmo
{
    ...
    private static void DrawGizmo(TrafficHeadquarter headquarter, GizmoType gizmoType)
    {
        ...
        foreach (TrafficSegment segment in headquarter.segments)
        {
            ...
            {
                //현재 웨이포인트 위치 찾고,
                Vector3 pos = segment.Waypoints[j].GetVisulPos();
                ...
                {
                    pNext = segment.Waypoints[j + 1].GetVisulPos();
                }
                //다음 웨이포인트가 있다면
            ...
            foreach (TrafficSegment nextSegment in segment.nextSegments)
            {
                if (nextSegment != null)
                {
                    Vector3 p1 = segment.Waypoints.Last().GetVisulPos();
                    Vector3 p2 = nextSegment.Waypoints.First().GetVisulPos();
                    //노란색 선으로 그려줍니다.
